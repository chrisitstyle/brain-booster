...
function getPromptAndAnswer(
  ...
  if (answerWith === "term") {
    return { prompt: card.definition, correctAnswer: card.term };
  }
  return { prompt: card.term, correctAnswer: card.definition };
}
...
export default function TestGame({ flashcards, setId }: TestGameProps) {
  ...
  function updateGameState(nextState: Partial<TestGameState>) {
    setGameState((previousState) => ({ ...previousState, ...nextState }));
  }
  useSaveGameResultOnFinish({
    ...
    isResultSaved,
    onSaved: () => {
      updateGameState({ isResultSaved: true });
    },
  });
  function updateConfig(nextConfig: Partial<TestConfig>) {
    setGameState((previousState) => ({
      ...previousState,
      config: { ...previousState.config, ...nextConfig },
    }));
  }
  ...
  function handleWrittenCheck() {
    ...
    updateGameState({ isAnswered: true, score: isCorrect ? score + 1 : score });
  }
  ...
  function handleMatchingPromptClick(cardId: number) {
    ...
    updateGameState({ selectedPromptId: cardId });
  ...
  function handleMatchingAnswerClick(cardId: number) {
    if (matchedIds.includes(cardId) || mismatchIds.length > 0) return;
    updateGameState({ selectedAnswerId: cardId });
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between gap-4 border-b border-gray-200 pb-5">
            ...
            <select
              ...
              onChange={(event) =>
                updateConfig({ answerWith: event.target.value as AnswerWith })
              }
            >
              {" "}
          ...
          {[
            ...
            return (
              ...
              >
                ...
                <button
                  type="button"
                  className={`relative h-7 w-12 rounded-full transition ${isEnabled ? "bg-pink-500" : "bg-gray-300"}`}
                  onClick={() => toggleQuestionType(questionType)}
                >
                  {" "}
                  <span
                    className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow-sm transition ${isEnabled ? "left-6" : "left-1"}`}
                  />{" "}
                </button>{" "}
              </div>
            );
      ...
      <div key={currentQuestion.id} className="game-enter space-y-6">
        ...
        {currentQuestion.type === "trueFalse" && (
          <div className="space-y-4">
            ...
            {isAnswered && (
              <div
                className={`rounded-xl border p-4 text-sm ${selectedAnswer === (currentQuestion.isTrueStatement ? "True" : "False") ? "border-green-200 bg-green-50 text-green-700" : "border-red-200 bg-red-50 text-red-700"}`}
              >
                {" "}
                Correct answer:{" "}
                <span className="font-semibold">
        ...
        {currentQuestion.type === "written" && (
          <div className="space-y-3">
            ...
            {isAnswered && (
              <div
                className={`rounded-xl border p-4 text-sm ${normalizeAnswer(writtenAnswer) === normalizeAnswer(currentQuestion.correctAnswer) ? "border-green-200 bg-green-50 text-green-700" : "border-red-200 bg-red-50 text-red-700"}`}
              >
                {" "}
                Correct answer:{" "}
                <span className="font-semibold">
                ...
                {matchingPromptCards.map((card) => {
                  ...
                  return (
                    <Button
                      ...
                      className={`h-auto min-h-14 w-full min-w-0 justify-start whitespace-normal break-words rounded-xl px-4 py-3 text-left leading-relaxed disabled:opacity-100 ${isMatched ? "border-green-200 bg-green-50 text-green-700" : isMismatch ? "game-shake border-red-300 bg-red-50 text-red-700" : isSelected ? "border-pink-400 bg-pink-50 text-pink-600 ring-2 ring-pink-100" : "border-gray-200 bg-white text-gray-700 hover:border-pink-300 hover:bg-pink-50 hover:text-pink-600"}`}
                      onClick={() =>
