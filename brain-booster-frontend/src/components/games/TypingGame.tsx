...
export default function TypingGame({ flashcards, setId }: TypingGameProps) {
  ...
  function handleCheckAnswer() {
    ...
    if (normalizedUserAnswer === normalizedCorrectAnswer) {
      updateGameState({ answerStatus: "correct", score: score + 1 });
    } else {
      updateGameState({ answerStatus: "incorrect" });
    }
  }
  function handleDontKnow() {
    if (isAnswered) return;
    updateGameState({ answerStatus: "revealed" });
  }
  function handleNext() {
        ...
        <div className="relative rounded-2xl border border-pink-100 bg-pink-50/40 p-5">
          ...
          <div className="pr-28">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-500">
              ...
              <span>•</span> <span>Score: {score}</span>{" "}
            </div>{" "}
